...
const ServerName = "atlassian-markdown-mcp"
...
// Server builds an *mcpsdk.Server with every registered tool wired in.
// It is exposed so tests can inspect the resulting server without touching
// stdio.
...
// per the MCP convention of returning IsError=true with a TextContent payload.
